...
namespace KIS.App_Code
{
    ...
    public class User
    {
        ...
        public User(String usr, String pwd)
        {
            ...
            String strSQL = "SELECT userID, nome, cognome, tipoUtente, lastLogin, ID, language FROM users WHERE userID LIKE '" + usr + "' AND password = MD5('" + pwd + "')";
            MySqlConnection conn = (new Dati.Dati()).mycon();
            conn.Open();
            MySqlCommand cmd = new MySqlCommand(strSQL, conn);
            MySqlDataReader rdr1 = cmd.ExecuteReader();
        ...
        public User(String usr)
        {
            this._homeBoxes = null;
            this._authenticated = false;
            String strSQL = "SELECT userID, nome, cognome, tipoUtente, lastLogin, ID, language FROM users WHERE userID LIKE '" + usr + "'";
            MySqlConnection conn = (new Dati.Dati()).mycon();
            conn.Open();
            MySqlCommand cmd = new MySqlCommand(strSQL, conn);
            MySqlDataReader rdr1 = cmd.ExecuteReader();
        ...
        public User(int IDn)
        {
            this._homeBoxes = null;
            this._authenticated = false;
            String strSQL = "SELECT userID, nome, cognome, tipoUtente, lastLogin, ID, language FROM users WHERE ID = " + IDn.ToString();
            MySqlConnection conn = (new Dati.Dati()).mycon();
            conn.Open();
            MySqlCommand cmd = new MySqlCommand(strSQL, conn);
            MySqlDataReader rdr1 = cmd.ExecuteReader();
