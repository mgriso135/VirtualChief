...
namespace KIS.Controllers
{
    ...
    public class KanbanBoxReaderController : ApiController
    {
        ...
        public IHttpActionResult Get(String tenant)
        {
            ...
            return Ok(Main(wsName));
        }

        // POST: api/KanbanBoxReader/Process
        [HttpPost]
        public IHttpActionResult Process(String tenant)
        {
            String wsName = WebEnv.ActiveWorkspaceName;
            if (wsName.Length == 0)
            {
        ...
        public int AddToProductionQue(String tenant, KanbanCard card)
        {
            ...
            ElencoCommesse elComm = new ElencoCommesse(wsName);
            elComm.loadCommesse();
            ...
            {
                var = new variante(wsName, prc.variantiProcesso[0].idVariante);
                procVar = new ProcessoVariante(wsName, prc, var);
                procVar.loadReparto();
            ...
            if (ret == 1)
            {
                ...
                Commessa cm = new Commessa(wsName, idCommessa, annoCommessa);
                ...
                if (cm.ID != -1 && cm.Year != 1)
                {
                    ...
                    {
                        Articolo art = new Articolo(wsName, prod[0], prod[1]);
                        ...
                        {
                            TaskVariante tskVar = new TaskVariante(wsName, new processo(wsName, art.Proc.process.subProcessi[i].processID, art.Proc.process.subProcessi[i].revisione), art.Proc.variant);
                            tskVar.loadTempiCiclo();
                            TempoCiclo tc = new TempoCiclo(wsName, tskVar.Task.processID, tskVar.Task.revisione, art.Proc.variant.idVariante, tskVar.getDefaultOperatori());
                            if (tc.Tempo != null)
                            {
                                lstTasks.Add(new TaskConfigurato(wsName, tskVar, tc, rp.id, art.Quantita));
                            }
                        }

                        ConfigurazioneProcesso prcCfg = new ConfigurazioneProcesso(wsName, art, lstTasks, rp, art.Quantita);
                        ...
                        if (rt1 == 1 && consistenza == 1)
                        {
                            ...
                            if (rt == 1)
                            {
                                ...
                                Boolean cambioStato = ChangeStatus(wsName, card, "process");
                                if (cambioStato == true)
                                {
