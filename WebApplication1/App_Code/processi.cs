...
namespace KIS
{
    ...
    public class processo
    {
        ...
        public bool attivo
        {
            ...
            set
            {
                ...
                conn.Open();
                MySqlCommand cmd = conn.CreateCommand();
                cmd.CommandText = "UPDATE processo SET attivo = 0 WHERE processID = " + this.processID.ToString()
                    ...
                    if (res != -1 && this.isVSM)
                    {
                        ...
                        if (this.subProcessi.Count > 1)
                        {
                            ...
                            if (indLast != -1)
                            {
                                ...
                                this.subProcessi[indLast].addProcessoSuccessivo(corrente, var);
                            }
                        }
                    }
        ...
        public bool addVariante(variante var)
        {
            ...
            if (this.processID != -1)
            {
                ...
                MySqlTransaction trans = conn.BeginTransaction();
                MySqlCommand cmd = conn.CreateCommand();
                cmd.CommandText = "INSERT INTO variantiprocessi(variante, processo, revProc) VALUES(" + var.idVariante.ToString() + ", " + this.processID.ToString() + ", " + this.revisione.ToString() + ")";
                try
                {
                    cmd.ExecuteNonQuery();
                    ret = true;
       ...
       public int delete()
       {
         ...
         * 0 if error
         */
        public bool addProcessoSuccessivo(processo next, variante var)
        {
            bool res = false;
            if(this.processID != -1)
            {
                String strSQL = "INSERT INTO precedenzeprocessi(prec, revPrec, succ, revSucc, variante, relazione) VALUES(" + 
                    this.processID.ToString() + ", " + this.revisione.ToString() + ", " + 
                    next.processID.ToString() + ", " + next.revisione.ToString() + ", " + var.idVariante + ", 0)";
                MySqlConnection conn = (new Dati.Dati()).mycon();
                conn.Open();
                MySqlTransaction tr = conn.BeginTransaction();
                MySqlCommand cmd = new MySqlCommand(strSQL, conn);
         ...
         * 0 if error
         */
        public bool addProcessoPrecedente(processo preced, variante var)
        {
            bool res = false;
            if (this.processID != -1)
            {
    ...
    public class variante
    {
        ...
        public bool DeleteLinkToProcesso(processo prc)
        {
            ...
            prcVar.process.loadFigli(new variante(this.idVariante));
    ...
    public class ProcessoVariante
    {
        ...
        public ProcessoVariante(processo prc, variante vr)
        {
            ...
            conn.Open();
            MySqlCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT * FROM processo INNER JOIN variantiprocessi ON(variantiprocessi.processo = processo.processID "
        ...
        public bool CopyTo(processo dest, bool copiaTasks, bool copiaTempiCiclo, bool copiaReparti, bool copiaPostazioni)
        {
            bool rt = true;
                            ...
                            else
                            {
                                ...
                                if (checkCopiaProcessi == true)
                                {
                                    ...
                                    for (int i = 0; i < this.process.subProcessi.Count && checkCopiaPrecedenze == true; i++)
                                    {
                                        ...
                                        {
                                            checkCopiaPrecedenze = nuovoProcVar.process.subProcessi[i].addProcessoSuccessivo(new processo(this.process.subProcessi[i].processiSucc[j]), var);
                                        }
                                    }
                                }
         ...
         */
        public int CopyTo(processo dest, String nomeVariante, String descVariante, bool copiaTasks, bool copiaTempiCiclo, bool copiaReparti, bool copiaPostazioni)
        {
            bool rt = true;
            int retVarID = -1;
            if (!(copiaPostazioni == true && copiaReparti == false))
