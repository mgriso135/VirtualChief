...
namespace KIS.App_Code
{
    public class Cliente
    {
         ...
         * Contact ID if ok
         */
        public int AddContatto(String nome, String role)
        {
            int rt = -1;
            if (this.CodiceCliente.Length > 0)
            {
                ...
                MySqlTransaction tr = conn.BeginTransaction();
                cmd.Transaction = tr;
                cmd.CommandText = "INSERT INTO contatticlienti(idContatto, cliente, nominativo, ruolo) VALUES("
                    + maxID.ToString() + ", "
                    + "'" + this.CodiceCliente + "', "
                    + "'" + nome + "', "
                    + "'" + role + "'"
    ...
    public class PortafoglioClienti
    {
        ...
        public bool Add(String codice, String ragSoc, String pIva, String codFiscale, String indirizzo, String citta, String provincia, String CAP, String stato, String telefono, String email, Boolean kanban)
        {
            ...
            validatePIvacFisc = ((codFiscale.Length > 0 || pIva.Length > 0) && codFiscale.Length < 255 && pIva.Length < 255) ? true : false;
    ...
    public class Contatto
    {
        ...
        public String Nominativo
        {
            ...
            set
            {
                if (value.Length > 0 && this.ID!=-1)
                {
                    ...
                    MySqlTransaction tr = conn.BeginTransaction();
                    cmd.Transaction = tr;
                    cmd.CommandText = "UPDATE contatticlienti SET nominativo = '" + value + "' WHERE idContatto = " + this.ID.ToString();
                    try
                    {
                        cmd.ExecuteNonQuery();
                        tr.Commit();
        ...
        public Contatto(int idC)
        {
            ...
            if (idC != -1)
            {
                ...
                conn.Open();
                MySqlCommand cmd = conn.CreateCommand();
                cmd.CommandText = "SELECT idContatto, cliente, nominativo, ruolo FROM contatticlienti WHERE idContatto = " + idC.ToString();
                MySqlDataReader rdr = cmd.ExecuteReader();
                if (rdr.Read())
                {
                    this._ID = rdr.GetInt32(0);
                    this._Cliente = rdr.GetString(1);
                    this._Nominativo = rdr.GetString(2);
                    this._Ruolo = rdr.GetString(3);
                    loadEmails();
                    loadPhones();
                }
                rdr.Close();
