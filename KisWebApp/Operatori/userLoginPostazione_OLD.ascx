<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="userLoginPostazione_OLD.ascx.cs" Inherits="KIS.Operatori.userLoginPostazione" %>
...
<asp:Repeater runat="server" ID="rptPostazioni" OnItemDataBound="rptPostazioni_ItemDataBound" OnItemCommand="rptPostazioni_ItemCommand">
    ...
    <ItemTemplate>
        ...
        <td><asp:ImageButton ToolTip="<%$Resources:lblCheckIn %>" runat="server" ID="btnCheckIn" ImageUrl="~/img/iconCheckIn4.jpg" Height="40" CommandName="checkIn" CommandArgument='<%#DataBinder.Eval(Container.DataItem, "ID") %>' />
        <asp:HiddenField runat="server" ID="id" Value='<%#DataBinder.Eval(Container.DataItem, "ID") %>' /></td>
